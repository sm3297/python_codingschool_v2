export const stages = [
  ...
  },
  {
    ...
    missions: [
      ...
      {
        id: "stage2-mission-final-test",
        title: "변수 종합 평가: 마스터의 자격",
        image: "/images/s2_m_final_test.png",
        rewardCoins: 400,
        rewardExp: 400,
        objective: "지금까지 배운 변수의 핵심 개념(할당, 자료형, 이름 규칙)을 모두 종합하여 스스로 문제를 해결할 수 있다.",
        story: "드디어 2단계의 마지막 필수 관문입니다! 변수 마스터가 되기 위한 최종 시험이 기다리고 있습니다. 아래의 빈칸(???)을 채워 코드를 완성하고 여러분의 실력을 증명해 보세요!",
        concept: "지금까지 배운 내용 총정리!\n1. 올바른 변수 이름 짓기 (스네이크 케이스)\n2. 정수(int)와 문자열(str) 구분하기\n3. type() 함수로 자료형 확인하기",
        analogy: "지금까지 모은 무기와 방어구, 물약(개념들)을 모두 활용해서 최종 보스를 물리치는 마지막 스테이지와 같습니다.",
        code: '# 1. 올바른 스네이크 케이스로 변수 이름을 지어 내 나이를 정수(int)로 저장하세요.\n??? = 14\n\n# 2. 내 닉네임을 문자열(str)로 저장하세요.\nmy_nickname = ???\n\n# 3. 위에서 만든 변수의 자료형을 확인하는 마법의 돋보기 함수를 쓰세요.\nprint(???(my_nickname))\n\nprint("변수 마스터 시험 통과!")',
        explanation: [
          "빈칸(???)을 스스로 생각해서 채워 넣어야 코드가 실행됩니다.",
          "단어 사이를 밑줄(_)로 잇는 이름 규칙을 기억하세요.",
          "글자를 저장할 때는 따옴표가 필요합니다.",
          "자료형을 확인하는 함수는 t로 시작합니다."
        ],
        practice: "위 코드의 ??? 부분을 모두 알맞은 정답으로 채워서 에러 없이 실행되게 만들어 보세요.",
        challenge: "나의 키를 실수(float)로 저장하는 변수를 추가하고, 그 변수의 타입도 함께 출력해보세요.",
        quizzes: [
          {
            question: "변수의 자료형을 알려주는 함수는 무엇인가요?",
            options: ["type()", "print()", "int()"],
            answer: 0
          },
          {
            question: "문자열(str) 데이터를 변수에 저장할 때 반드시 필요한 것은 무엇인가요?",
            options: ["따옴표 (' ' 또는 \" \")", "밑줄 (_)", "숫자"],
            answer: 0
          }
        ]
      }
